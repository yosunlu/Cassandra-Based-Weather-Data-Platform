...
"""
Server for gRPC. Why use cassandra for the table?
...
The table created in note book does not have date and name.
Use RecordTemps (given id, date, station_record) to fill in the tables.
...
class ModelServer(station_pb2_grpc.StationServicer):
    def __init__(self):
            ...
            VALUES(?, ?, ?) 
            """)
            # ?, ?, ? will correspond to (request.station, request.date, Record(request.tmin, request.tmax)) at line 40
    ...
    def StationMax(self, request, context):
        ...
        """
        try:
            rows = cass.execute(self.max_statement, [request.station]) # turning the df in to a list; the argument for ? must be a list, even if there's only one
            sorted_rows = sorted(rows, key=lambda x: x.record.tmax, reverse=True) # sort the list based on tmax
            return station_pb2.StationMaxReply(tmax=sorted_rows[0].record.tmax, error="") # sorted_rows[0] will be the first row of the station id
